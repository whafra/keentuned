...
class Benchmark():
    ...
    def run(self):
        ...
        if result.returncode == 0:
            ...
            TPCH_time_measurement = re.search(pattern_TPCH_time,self.out).group(2)
            TPCH_time = self.__transfMeasurement(_TPCH_time, TPCH_time_measurement)
            
            _Total_time = float(re.search(pattern_Total_time,self.out).group(1))
            Total_time_measurement = re.search(pattern_Total_time,self.out).group(2)
            Total_time = self.__transfMeasurement(_Total_time, Total_time_measurement)
