...
export default () => {
  ...
  const cmdOperation = async (q: any, operateType: string)=> {
    ...
    setLoading(true);
    try {
      const data = await getRequestData(q) || {}
      setLoading(false);
      if (data.suc) {
        message.success(`${operateType}成功`);
        requestAllData()
      } else {
        dataDealError(data, `${operateType}失败！`)
      }
    } catch (err) { 
      setLoading(false);
    }
  ...
  const fn = (key: string, row: any)=> {
    switch (key) {
      ...
      // case 'details': viewDetails(`/var/keentune/profile/${row.name}`, row.name); break
      ...
      <PageContainer
        ...
        <ProTable<TableListItem>
          ...
          pagination={{
            ...
            showTotal: (total, range) => { return `共 ${total} 条记录 第 ${listPage.pageNum} / ${Math.ceil(total / listPage.pageSize)} 页`},
